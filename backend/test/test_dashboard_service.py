...
def _make_cuenta(saldo: Decimal) -> MagicMock:
    cuenta = MagicMock()
...
class TestGetDashboardSummary:
    def _setup_db(self, mock_db, usuario=None, cuentas=None, meta=None, oportunidad=None):
        ...
        # Cuentas → filter().all()
        q.filter.return_value.all.return_value = cuentas
    ...
    def test_score_y_flujo_por_defecto_si_usuario_no_existe(self, mock_db):
        self._setup_db(mock_db, usuario=None, cuentas=[])
    ...
    def test_saldo_total_es_suma_de_cuentas(self, mock_db):
        usuario = MagicMock()
        usuario.score_resiliencia = 500
        usuario.flujo_caja_libre_mensual = Decimal("0")
        cuentas = [_make_cuenta(Decimal("1000")), _make_cuenta(Decimal("500")), _make_cuenta(Decimal("250"))]

        self._setup_db(mock_db, usuario=usuario, cuentas=cuentas)

        resultado = svc.get_dashboard_summary(mock_db, uuid4(), "user_1")

        assert resultado["saldo_total"] == Decimal("1750")

    def test_saldo_total_es_cero_sin_cuentas(self, mock_db):
        usuario = MagicMock()
        usuario.score_resiliencia = 500
        usuario.flujo_caja_libre_mensual = Decimal("0")

        self._setup_db(mock_db, usuario=usuario, cuentas=[])
